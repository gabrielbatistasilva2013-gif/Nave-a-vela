      ...
      <div className={`fixed inset-0 z-50 pointer-events-none flex transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] ${isNavTop ? 'items-start justify-center pt-6' : 'max-md:items-end max-md:justify-center max-md:pb-6 md:items-center md:justify-end md:pr-8'}`}>
        ...
        >
          <button
            ...
            className={`flex items-center justify-center p-2 rounded-full transition-all duration-300 text-xs font-bold uppercase tracking-widest group relative ${activeTab === 'home' ? 'text-blue-50 bg-blue-500/20 ring-1 ring-blue-500/50 shadow-[0_0_20px_rgba(59,130,246,0.4)]' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}
            title="Início"
          >
            {activeTab === 'home' && (
              <span className="absolute inset-0 rounded-full animate-pulse bg-blue-400/20 shadow-[0_0_15px_rgba(96,165,250,0.6)] mix-blend-screen pointer-events-none"></span>
            )}
            <Globe className={`w-5 h-5 shrink-0 transition-colors z-10 relative ${activeTab === 'home' ? 'text-blue-300' : 'group-hover:text-blue-400'}`} />
